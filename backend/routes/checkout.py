...
from backend.agent.schemas import AuthorizeCheckoutRequest, CheckoutRequest, FailureRequest, VerifyPaymentRequest
...
from backend.services.checkout_service import verify_checkout_payment
from backend.services.razorpay_service import handle_payment_failure
...
def failure():
    try:
        ...
        return fail(exc.errors()[0]["msg"], 422, "VALIDATION_ERROR")
    result = handle_payment_failure(payload.razorpay_order_id, payload.reason)
    for event_type in ["PAYMENT_ATTEMPTED", "PAYMENT_FAILED", "CART_PRESERVED", "DUPLICATE_EXECUTION_PREVENTED", "USER_NOTIFIED"]:
